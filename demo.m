...
names = {'PreCoffee' }; %'PreAdiac', 'PreBeef', 'PreFISH', 'PreOliveOil', 'PreOSULeaf',...
    % 'PreCar', 'PreCoffee', 'PreLighting2', 'PreSymbols'};
R_nos = [200,300];
dataSetIndex = 1;
name = names{dataSetIndex};

R_no = R_nos(dataSetIndex); % 200;%number of the reservior units, need to set
...
[normal_testAcc] = NormalRV(name,lambdas,R_no,nForgetPoints,costs,kps,0); 
% the parameter 0 indicates a analogue series, otherwise 1

%[GMM_testAcc] = GMMRV(name,lambdas,R_no,nForgetPoints,costs,kps,0);

%[fisher_testAcc] = fisherRV(name,lambdas,R_no,nForgetPoints,costs,kps,0);
